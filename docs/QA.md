...
## Reviewed matches and company logos — 18 September 2026

- Reviewed 14 additional records against full employer/LinkedIn descriptions, including one separately verified Capgemini Supply Planning Specialist. Merged the successful 18 September scheduled scan (20 additional discoveries) without replacing history. Reviewed eight of those additional records. Final dataset: 50 records, 28 scored, 23 active scored recommendations; eight closed, 14 unscored pending descriptions, five held (two possible HP duplicates, one employer-identity mismatch, one elapsed recommended apply-by date, one out-of-region lead). A failed fetch is not treated as closure.
- Confirmed closed/expired: Be Verse Demand Supply Planner, Capgemini Demand Planner, Hired Procurement Specialist, Hired Supply Chain Manager (expired-listing redirect), Vestas Inventory Count Analyst (10 September deadline), Be Verse Buyer and Be Verse Supply Planner. Ford was already closed.
- Corrected employer identities and titles. ConverSight permits Chennai in its description despite the Coimbatore header; Chennai base/travel must be confirmed. Python/SQL gaps are explicit. Rockwell is Kanchipuram in the Chennai manufacturing region.
- Every Jobs card and scored Saved card displays a percentage, including LinkedIn. Unscored discoveries are excluded from Jobs/Resume and described under Sources. Existing private tracking records are retained. A possible HP duplicate is held rather than inflating the active count.
- Official company marks are served locally. Asset origins in COMPANY_LOGOS.md; unknown historical employers use a neutral briefcase, never an initial. Exact build allowlist extended by explicit filenames; no third-party logo requests or new credentials.
- Deadline parser now supports day-first ordinal dates, including “apply on or before 10th September 2026”; invalid dates and recommendation snippets are tested.
- Passed 29 Node tests and 33 Python tests, dataset validation and public-only privacy build.
- Local browser checks at 390×844 and 1280×900: 23/23 active cards have scores and loaded company images, no horizontal overflow; mobile detail and weighted breakdown verified; no console warnings/errors. Sources exposes specific unscored limitations. Target salary banner removed from job headers as previously requested.
